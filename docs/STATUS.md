...
2. **Phase 2 (Sentinel-1 SAR — provider + `sar_seed.py` + first reviewed training set (`sar_training_exports/deep/`, 149 chips / 206 boxes) all built; still need a SAR base + build/train flow to produce `best_sar.pt`)** for free all-weather / dark-vessel coverage, or **Phase 3 (AIS dark-vessel reframe)** —
