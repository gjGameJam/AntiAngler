...
_Snapshot: 2026-07-17. Point-in-time; verify against code before trusting specifics._
...
- **Phase 3 (AIS dark-vessel fusion) STARTED — the matcher is built (2026-07-22).** `scripts/fuse_violations.py` (pure stdlib) reads a run's `detections.json` + a normalized AIS positions file (our own source-agnostic `{mmsi,lon,lat,timestamp,sog?,cog?}` schema, JSON or CSV) and, per **in-MPA** detection, decides **dark vs matched** with a **velocity-aware feasible-movement envelope** (`reach = sog·Δt + slack`, haversine ≤ reach, `max-speed` fallback when sog missing/zero — not a fixed radius). Unmatched → **dark candidate** (the product signal, `detection − AIS`); emits `violation_events.json` (CLAUDE.md schema; `fishing_probability`/`fishing_hours` null until the GFW join) + `violation_events.geojson` (dark points), atomic. Validated **offline** by `scripts/tests/test_fuse_violations.py` (9 tests incl. the roadmap's 3 verification cases — all pass; no network / no ML deps). **Step 2 — the AISStream ingester — is also built (2026-07-22):** `scripts/aisstream_fetch.py` (its own module; never imports the matcher) subscribes to a bbox/MPA on the free AISStream websocket, collects `PositionReport`s for a bounded `--duration`, and writes the normalized contract to `data/raw/ais/` for the matcher. Wire-format gotchas handled + offline-tested (`test_aisstream_fetch.py`, 19 tests incl. a stubbed-websocket collect loop + a normalize→write→fuse round trip): `[lat,lon]` bbox order, Go nanosecond `time_utc`, `Sog`/`Cog` sentinels→null. Only extra dep `websocket-client` (`requirements-ais.txt`, lazy). AISStream is live-only (near-real-time workflow; historical scenes need an archive/Spire source later). **Step 3 — the GFW vessel/fishing join — is also built (2026-07-22):** `scripts/gfw_vessels.py` (its own stream; never imports the matcher) resolves MMSIs (explicit, or pulled from a `violation_events.json`/AIS file) to GFW identity (Vessels search) + fishing hours (Events API) + optional Insights (`--insights`: IUU/AIS-off/no-take-MPA fishing), writing normalized vessel records to `data/raw/gfw_vessels/`. `fuse_violations.py` gained a **strictly-additive `--vessels` join** that enriches matched events (identity + `fishing_hours`/`fishing_probability`, `"GFW"` evidence) and lifts `violation_score` (matched weight interpolates `--matched-weight`→`--fishing-weight`; IUU escalates to full) — **verdicts/scores unchanged from the AIS-only path when `--vessels` is absent** (event gains null GFW fields; pre-existing fuse tests unchanged). Offline-tested (`test_gfw_vessels.py` 16 + 6 new enrichment tests; **50 P3 tests total**, no network/token). ⚠️ **The GFW v3 HTTP shapes follow the docs but are NOT live-verified** (no token here, same as `gfw_fetch.py`) — parsers are tolerant, `--dry-run` prints the plan; verify on first real run. **Remaining P3 (optional polish):** Skylight dark-call cross-check, a per-MPA batch driver, and tightening `fishing_probability` to location-specific. See `docs/ROADMAP.md` Phase 3.
