...
- **Phase 3 (AIS dark-vessel fusion) — all three stages BUILT & offline-tested (2026-07-22).** The product loop `fuse_violations.py` (matcher: dark-vs-matched via a velocity-aware feasible-movement envelope) + `aisstream_fetch.py` (AISStream → normalized AIS contract, live-only) + `gfw_vessels.py` (GFW identity/fishing/Insights) + the strictly-additive `fuse_violations.py --vessels` enrichment. **50 offline tests**, no network/key/ML deps; each ingester is its own module (never imports the matcher). ⚠️ **The AISStream + GFW calls are NOT yet live-verified** — this remote sandbox blocks the AISStream/GFW hosts (egress policy) and has no keys/token, so live validation must run elsewhere (a machine/env that allowlists the hosts, with the key + token); parsers are tolerant and `--dry-run` prints the plan. Verifying them live and producing a first real violation is what remains. **Remaining work + definition of done: `docs/PHASE3_PLAN.md`.** Per-script contracts: `CLAUDE.md`.
