...
2. **GPU fine-tuning** via the AntiAngler venv (`venv/Scripts/python.exe`), ultralytics 8.4.90 / **torch 2.12.1+cu130 on the RTX 4070** (verified 2026-08-03; `train.py --device` auto-detects CUDA). A 30-epoch 1024px P2 run takes **~25 min** — the old ~5 min/epoch CPU figure is obsolete. Still: **keep the machine awake**, sleep inflates wall-clock massively.
