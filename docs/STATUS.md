...
- **Live model:** `data/training/weights/best.pt` = the **`finetune-bigship`** run (promoted 2026-07-26), **deployed at `imgsz 1024`, `conf 0.20`** — the YOLOv8n-**P2** arch retrained with the Gulf-of-Finland S2 set folded in (train 1,475 / val 422), warm-started from `finetune-smallvessel`, **trained on GPU** (RTX 4070, cu130; ~47 s/epoch vs ~26 min on CPU). `detect_boats.py` defaults bake this in. Backups: `best_finetune-smallvessel.pt` (prior live), `best_finetune-openocean.pt`, `best_finetune-p2-1024.pt`, `best_finetune-aug-1024.pt`, `best_finetune-aug-640.pt`, `best_multiscene.pt`, `best_baseline_pre-multiscene.pt` (see the Model progression table).
...
| **`finetune-bigship` (07-26)** | +Gulf-of-Finland S2 (1,034 chips / 7,984 boxes) | Alonnisos @0.20: P 0.48/R **0.76** (was 0.40/0.70 @0.15); Port Said P 0.94/R 0.71; SG improved⁶ | ✅ **LIVE** (deploy conf **0.20**) |
...
## Current training data (the live model — 10 reviewed scenes + Finland-s2)
