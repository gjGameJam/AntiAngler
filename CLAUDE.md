...
- `data/raw/ais/` — Normalized AIS position snapshots written by `aisstream_fetch.py` as `ais_positions_<UTC-timestamp>_<label>.json`: `{generated_utc, source, bbox_wgs84, wdpa, positions:[{mmsi, lon, lat, timestamp, sog, cog, name?}]}`. The `positions` list is the AIS input contract `fuse_violations.py --ais` reads. Gitignored.
