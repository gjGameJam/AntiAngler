...
# Step 10 (P3 fusion): match a run's detections against normalized AIS positions with a
# velocity-aware gate; write violation_events.json (+ .geojson of dark points). Pure stdlib.
python scripts/fuse_violations.py --run data/raw/sentinel2/<run>/ --ais positions.json

# Run the offline fusion tests (synthetic fixtures; no network / no ML deps)
